...
class Client {
  ...
  request (service, params) {
    return new Promise((resolve, reject) => {
      debug('client.request service %s with params=%o', service, params)
      const requester = new Rabbus.Requester(this.rabbot, service)
      requester.request(params, function (response) {
        if (response.eraro === true) {
          reject(response)
        } else {
          resolve(response)
        }
  ...
  subscribe (event, handler) {
    return new Promise((resolve, reject) => {
      debug('client.subscribe event %s', event)
