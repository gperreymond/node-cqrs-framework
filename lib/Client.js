...
class Client {
  ...
  request (name, data, callback) {
    const queueName = name + '.Response.' + uuid.v4()
    this.debug('requester %s has been sent with data %o', name, data)
    this.bus.subscribe({
      routingKey: name,
      queueName
    }, response => {
      this.debug('responser %s has been called with data %o', name, data)
      this.bus.unsubscribe(queueName)
      callback(response)
    })
    data.__response = queueName
    this.bus.send(name, data)
  }
  start (options) {
