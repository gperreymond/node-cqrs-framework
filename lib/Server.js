  ...
  initialize () {
    return new Promise((resolve, reject) => {
      ...
      this.rabbot.on('unreachable', () => {
        const errorMessage = this.error('server_error_no_bus_connected', {uuid: this.uuid, type: this.type, name: this.name, error: new Error('Rabbitmq unreachable')})
        debug(errorMessage)
        reject(errorMessage)
      })
      ...
      this.rabbot
        ...
        .catch((error) => {
          const errorMessage = this.error('server_rabbot_error', {uuid: this.uuid, type: this.type, name: this.name, error: error})
          debug(errorMessage)
          reject(errorMessage)
        })
    })
        ...
        _.map(files, (filepath) => {
          ...
          if (typeof handler === 'function') {
            ...
            if (type !== 'None') {
              ...
              // responders
              let responder = new Rabbus.Responder(this.rabbot, name)
