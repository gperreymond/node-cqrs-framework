  ...
  initialize () {
    return new Promise((resolve, reject) => {
      ...
      this.bus.on('connected', () => {
        ...
        this.start().then(() => {
          debug('server.client try to initialize the bus %o', this.options.bus)
          this.client = new Client(this.options.bus)
          this.client.rabbot = this.rabbot
          this.client.initialize().then(resolve).catch(reject)
        }).catch(reject)
      })
      this.bus
