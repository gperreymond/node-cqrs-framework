...
class Server {
  ...
  receiver (name, handler) {
    this.debug('receiver %s has been added', name)
    this.__receivers[name] = handler
    return this
  }
  start (options) {
    this.debug('listen')
...
Server.prototype.use = function (dirpath) {
  ...
  files.map(filepath => {
    ...
    if (type === 'Command') this.receiver(name, new Command(name, require(filepath)))
    if (type === 'Query') this.receiver(name, new Query(name, require(filepath)))
  })
  return this
}
